...
class data:
    ...
    def makeWaveFig(self):
        ...
        self.waveFig.subplots_adjust(top=0.9, right=0.98, bottom=0.15)
    ...
    def makeFreqFig(self):
        ...
        self.freqFig.subplots_adjust(top=0.9, right=0.98, bottom=0.15)
        ...
        self.freqAx.scatter(self.freqData, self.freqDataAbsDisp, marker='.')
    ...
    def runFFT(self):
        ...
        self.freqDataAbsDisp = self.freqDataAbs
        self.freqDataAng = np.angle(self.compFreqData, deg=True)
    
    def exportCSV(self):
        np.savetxt('./output.csv', np.stack([np.array(self.freqData), self.freqDataAbsDisp]).T, delimiter=',')
        
    def yAxisLog(self):
        self.freqDataAbsDisp = 20 * np.log10(self.freqDataAbs)

    def yAxisAmp(self):
        self.freqDataAbsDisp = self.freqDataAbs
        
    def yAxisRMS(self):
        self.freqDataAbsDisp = self.freqDataAbs/np.sqrt(2)
...
frameFreq = [[sg.Radio('Amp.', group_id='yAxis', key='-yAxisAmp-', enable_events=True,default=True, disabled=True), sg.Radio('RMS', group_id='yAxis', key='-yAxisRMS-', enable_events=True, disabled=True), sg.Radio('Amp.dbV', group_id='yAxis', key='-yAxisLog-', enable_events=True, disabled=True)], [sg.Canvas(key='-freqCanvas-', size=(400, 300))],[sg.Text('表示最大振幅'), sg.InputText(key='-ampRange-', enable_events=True, justification='right', size=(5,1)), sg.Button('Apply', key='-reloadAmpRange-', disabled=True, enable_events=True), sg.Text('表示最大周波数'), sg.InputText(key='-freqRange-', enable_events=True, justification='right', size=(5,1)), sg.Button('Apply', key='-reloadFreqRange-', disabled=True)]]
...
while True:
    ...
    if event in (None, 'Cancel'):
        ...
        window.find_element('-yAxisLog-').Update(disabled=False)
        window.find_element('-yAxisAmp-').Update(disabled=False)
        window.find_element('-yAxisRMS-').Update(disabled=False)
